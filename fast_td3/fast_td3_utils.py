...
class SimpleReplayBuffer(nn.Module):
    def __init__(
        ...
        self.amp_observations = torch.zeros(
            (n_env, buffer_size, n_amp_obs), device=device, dtype=torch.float
        )
        self.ptr = 0
    ...
    def extend(
        ...
        amp_observations = tensor_dict["amp_observations"]
        ...
        self.amp_observations[:, ptr] = amp_observations
        self.ptr += 1
    ...
    def sample(self, batch_size: int):
        ...
        if self.n_steps == 1:
            ...
            amp_obs_indices = indices.unsqueeze(-1).expand(-1, -1, self.n_amp_obs)
            amp_observations = torch.gather(
                self.amp_observations, 1, amp_obs_indices
            ).reshape(self.n_env * batch_size, self.n_amp_obs)
        else:
            # Sample base indices
...
class EmpiricalNormalization(nn.Module):
    ...
    def update(self, x):
        ...
        with torch.no_grad():
            ...
            new_mean = self._mean + (batch_size / new_count) * delta
            ...
            if self.count > 0:  # Ensure we're not dividing by zero
                ...
                new_var = torch.mean((x - new_mean) ** 2, dim=0, keepdim=True)

            new_std = torch.sqrt(torch.clamp(new_var, min=self.eps**2))
...
class RewardNormalizer(nn.Module):
    ...
    def update_stats(
        ...
        with torch.no_grad():
            ...
            self.G_r_max = torch.maximum(self.G_r_max, torch.abs(self.G).max())
