...
from amp import MultiTaskDiscriminator
...
def main():
    ...
    from environments.isaaclab_amp_multitask_env import IsaacLabAMPMultiTaskEnv

    envs = IsaacLabAMPMultiTaskEnv(
        device.type,
        args.num_envs,
        args.seed,
    ...
    actor_kwargs = {
        "n_obs": n_obs - envs.num_tasks + args.task_embedding_dim,
        "n_act": n_act,
    ...
    critic_kwargs = {
        "n_obs": n_critic_obs - envs.num_tasks + args.task_embedding_dim,
        "n_act": n_act,
    ...
    discriminator_kwargs = {
        "n_amp_obs": n_amp_obs - envs.num_tasks + args.task_embedding_dim,
        "hidden_dim": args.discriminator_hidden_dim,
        "lsgan_reward_scale": args.lsgan_reward_scale,
        "device": device,
        "num_tasks": envs.num_tasks,
        "task_embedding_dim": args.task_embedding_dim,
    ...
    actor_detach = MultiTaskActor(**actor_kwargs)
    # Copy params to actor_detach without grad
    from_module(actor).data.to_module(actor_detach)
    policy = actor_detach.explore

    qnet = MultiTaskCritic(**critic_kwargs)
    qnet_target = MultiTaskCritic(**critic_kwargs)
    qnet_target.load_state_dict(qnet.state_dict())

    discriminator = MultiTaskDiscriminator(**discriminator_kwargs)
    ...
    def sample_amp_motion_obs(amp_observations, amp_motion_obs):
        # Task-aware sampling of amp_motion_obs
        motion_batch_size = amp_observations.shape[0]
        
        # Extract task indices from one-hot encoding in amp_observations
        # Task information is in the last num_tasks dimensions
        task_ids_one_hot = amp_observations[:, -envs.num_tasks:]
        task_indices = torch.argmax(task_ids_one_hot, dim=1)  # [batch_size]
        
        # Generate random motion indices for each sample
        motion_indices = torch.randint(
            0, amp_motion_obs.shape[1], (motion_batch_size,), device=device
        )
        return torch.cat([
            amp_motion_obs[task_indices, motion_indices],
            task_ids_one_hot,
        ], dim=1)
    ...
    if args.compile:
        ...
        sample_amp_motion_obs = torch.compile(sample_amp_motion_obs, mode=mode)
    ...
    with torch.no_grad():
        amp_motion_obs = torch.stack([
            envs.sample_task_reference_motions("dance", args.num_envs * args.buffer_size // 10),
            envs.sample_task_reference_motions("run", args.num_envs * args.buffer_size // 10),
            envs.sample_task_reference_motions("walk", args.num_envs * args.buffer_size // 10),
        ], dim=0)
    ...
    while global_step < args.total_timesteps:
        ...
        if args.reward_normalization:
            ...
            update_stats(online_rewards, dones.float(), task_ids=task_indices)
        ...
        if global_step > args.learning_starts:
            for i in range(args.num_updates):
                ...
                data["amp_motion_observations"] = sample_amp_motion_obs(
                    data["amp_observations"], amp_motion_obs
                )                
